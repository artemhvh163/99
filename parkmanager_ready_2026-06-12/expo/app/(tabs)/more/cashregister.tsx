...
import { calculateShiftCashBalance, calculateShiftClosingSummary } from '@/utils/financeCalculations';
...
export default function CashRegisterScreen() {
  ...
  const handleCloseShift = useCallback(() => {
    ...
    if (absDiscrepancy > 0.01 && !closeNote.trim()) {
      Alert.alert('Ошибка', 'Укажите комментарий к расхождению в кассе');
      return;
    }
  ...
  const handleAddExpense = useCallback(() => {
    ...
    if (!expenseDesc.trim()) {
      Alert.alert('Ошибка', 'Укажите комментарий к расходу');
      return;
    }
    const result = addExpense(amt, expenseCategory || 'Хоз. расходы', expenseDesc.trim());
    if (!result?.success) {
      ...
      {!currentShift || isBlockedByOther ? (
        ...
        <>
          <View style={styles.balanceCard}>
            ...
            <Text style={styles.balanceValue} numberOfLines={1} adjustsFontSizeToFit>{formatMoney(expectedCash)}</Text>
          ...
          <View style={styles.closeSection}>
            ...
            <TextInput
              style={styles.formInput}
              placeholder="Комментарий к расхождению"
              placeholderTextColor={colors.textTertiary}
              value={closeNote}
