...
export default function FinanceScreen() {
  ...
  const handleWithdraw = () => {
    ...
    if (!withdrawNote.trim()) {
      Alert.alert('Ошибка', 'Укажите комментарий к снятию');
      return;
    }
    const result = adminWithdrawFromManager(amt, withdrawNote.trim());
    if (!result?.success) {
      Alert.alert('Ошибка', result?.error ?? 'Не удалось выполнить снятие');
      return;
    }
    setWithdrawAmount('');
    setWithdrawNote('');
  ...
  const handleAddExpense = () => {
    ...
    const result = addAdminExpense(amt, expMethod, expCategory || 'Прочее', expDesc.trim());
    if (!result?.success) {
      Alert.alert('Ошибка', result?.error ?? 'Не удалось добавить расход');
      return;
    }
    setExpAmount('');
    setExpCategory('');
            ...
            {currentShift && (
              <View style={styles.managerCashCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.managerCashValue} numberOfLines={1}>{formatMoney(managerCashBalance)}</Text>
                </View>
              </View>
            )}
          </>
            ...
            {currentShift && (
              <View style={styles.managerCashCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.managerCashValue}>{formatMoney(managerCashBalance)}</Text>
                </View>
              </View>
            )}
            ...
            {currentShift ? (
              <View style={styles.formCard}>
                <Text style={styles.formHint}>
                  В кассе менеджера: {formatMoney(managerCashBalance)}
                </Text>
                <TextInput
