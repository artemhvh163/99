...
import { buildEntitySearchQuery, formatMoney, matchClientWithCars } from '@/utils/helpers';
...
export default function ClientsScreen() {
  ...
  const clients = useMemo(() => {
    const query = buildEntitySearchQuery(search);
    return activeClients
      ...
      .filter(item => {
        ...
        if (!query.hasText) return true;
        return matchClientWithCars(item.client, item.cars, query).matches;
      })
      .sort((a, b) => {
        if (!query.hasText) return a.client.name.localeCompare(b.client.name);
        const aExact = matchClientWithCars(a.client, a.cars, query).exact;
        const bExact = matchClientWithCars(b.client, b.cars, query).exact;
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  topBar: {
    flexDirection: 'row', gap: 10, padding: 16, paddingBottom: 10,
  },
  searchBox: {
  ...
  countLabel: {
    fontSize: 12, color: colors.textTertiary, paddingHorizontal: 16, paddingBottom: 8,
  },
  ...
  carsText: { fontSize: 12, color: colors.textSecondary, marginTop: 3 },
  debtRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 4 },
  debtText: { fontSize: 12, color: colors.danger, fontWeight: '600' as const },
  phoneRow: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: 5, marginTop: 3 },
