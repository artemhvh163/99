...
import {
  View, Text, StyleSheet, ScrollView, TouchableOpacity, RefreshControl, Alert, Animated, Easing, Modal, KeyboardAvoidingView, Platform
} from 'react-native';
...
export default function DashboardScreen() {
  ...
  const searchResults = useMemo(() => {
    ...
    const query = buildEntitySearchQuery(searchQuery);
    const partial = activeClients.filter(c => {
      const clientCars = activeCars.filter(car => car.clientId === c.id);
      return matchClientWithCars(c, clientCars, query).matches;
    });
    const exact = partial.filter(c => {
      const clientCars = activeCars.filter(car => car.clientId === c.id);
      return matchClientWithCars(c, clientCars, query).exact;
    });
        ...
        <View style={styles.shiftActiveCard}>
          <View style={styles.shiftActiveTop}>
            ...
            {isAdmin && currentShift.operatorRole === 'manager' && (
              <TouchableOpacity onPress={() => { setAdminCloseCash(String(Math.round(totalCashData?.totalCash ?? 0))); setAdminCloseNote(''); setShowAdminCloseModal(true); }} style={styles.adminCloseBtn}>
            ...
            <View style={styles.shiftModalBtns}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  if (!adminCloseNote.trim()) {
                    Alert.alert('Ошибка', 'Укажите причину принудительного закрытия смены');
                    return;
                  }
                  Alert.alert(
                    'Закрыть смену?',
                    ...
                    [
                      ...
                      { text: 'Закрыть', style: 'destructive', onPress: () => {
                        adminForceCloseShift(currentShift.id, cash, adminCloseNote.trim());
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  greetingTime: { fontSize: 13, color: colors.textSecondary, marginBottom: 2, letterSpacing: 0.1 },
  greeting: { fontSize: 22, fontWeight: '700' as const, color: colors.text, letterSpacing: -0.3 },
  role: { fontSize: 12, color: colors.textTertiary, marginTop: 3, letterSpacing: 0.2, textTransform: 'uppercase' as const },
  logoutBtn: { padding: 10, borderRadius: 12, backgroundColor: colors.surfaceLight },
  shiftBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.warningSurface, borderRadius: 14, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: colors.warning + '25' },
  ...
  shiftActiveCard: { backgroundColor: colors.primarySurface, borderRadius: 14, padding: 14, marginBottom: 16, borderWidth: 1, borderColor: colors.primary + '15' },
  ...
  shiftProgressTrack: { height: 3, backgroundColor: colors.border, borderRadius: 2, marginTop: 10, overflow: 'hidden' as const },
  shiftProgressBar: { height: 3, backgroundColor: colors.primary, borderRadius: 2 },
  shiftProgressLabel: { fontSize: 11, color: colors.textTertiary, marginTop: 6 },
  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 16 },
  statCard: { backgroundColor: colors.surface, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: colors.border },
  statIconWrap: { width: 38, height: 38, borderRadius: 11, alignItems: 'center', justifyContent: 'center', marginBottom: 10 },
  ...
  debtSummaryCard: { backgroundColor: colors.dangerSurface, borderRadius: 14, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: colors.danger + '15' },
  debtSummaryRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 },
  debtSummaryLabel: { fontSize: 14, color: colors.danger, fontWeight: '600' as const },
  debtSummaryValue: { fontSize: 24, fontWeight: '700' as const, color: colors.danger, letterSpacing: -0.3 },
  debtPaymentsToday: { fontSize: 12, color: colors.textSecondary, marginTop: 6 },
  quickLinks: { marginBottom: 20 },
  sectionTitle: { fontSize: 15, fontWeight: '600' as const, color: colors.text, marginBottom: 12, letterSpacing: -0.1 },
  quickGrid: { flexDirection: 'row', gap: 10 },
  quickBtn: { flex: 1, backgroundColor: colors.surface, borderRadius: 12, paddingVertical: 14, paddingHorizontal: 8, alignItems: 'center', gap: 8, borderWidth: 1, borderColor: colors.border },
  quickLabel: { fontSize: 11, color: colors.textSecondary, fontWeight: '500' as const, textAlign: 'center' as const },
  section: { marginBottom: 20 },
  expiringCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surface, borderRadius: 12, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: colors.border },
  ...
  expiringBadge: { backgroundColor: colors.warningSurface, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4 },
  ...
  violationCard: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: colors.surface, borderRadius: 14, padding: 14, marginBottom: 16, borderWidth: 1 },
  violationTitle: { fontSize: 14, fontWeight: '600' as const },
  violationDots: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 4 },
  violationDot: { width: 8, height: 8, borderRadius: 4 },
  violationCount: { fontSize: 11, color: colors.textTertiary, marginLeft: 4 },
  cleaningBannerNew: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: 12, backgroundColor: 'rgba(251, 191, 36, 0.10)', borderRadius: 14, padding: 16, marginBottom: 16, overflow: 'hidden' as const, position: 'relative' as const },
  cleaningGlowBorder: { position: 'absolute' as const, top: 0, left: 0, right: 0, bottom: 0, borderRadius: 14, borderWidth: 2 },
  cleaningIconWrap: { width: 42, height: 42, borderRadius: 12, backgroundColor: 'rgba(251, 191, 36, 0.18)', alignItems: 'center' as const, justifyContent: 'center' as const },
  cleaningTitleNew: { fontSize: 15, fontWeight: '700' as const, color: '#FBBF24', letterSpacing: -0.2 },
  cleaningSubNew: { fontSize: 12, color: colors.textSecondary, marginTop: 3 },
  cleaningArrow: { width: 32, height: 32, borderRadius: 8, backgroundColor: 'rgba(251, 191, 36, 0.12)', alignItems: 'center' as const, justifyContent: 'center' as const },
  searchContainer: { marginBottom: 14 },
  searchBar: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: colors.surface, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 11, borderWidth: 1, borderColor: colors.border },
  ...
  searchEmpty: { fontSize: 13, color: colors.textTertiary, textAlign: 'center' as const, paddingVertical: 14 },
  ...
  searchItem: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: 0.5, borderBottomColor: colors.border },
  ...
  totalCashCard: { backgroundColor: colors.surface, borderRadius: 14, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: colors.primary + '20' },
  totalCashHeader: { flexDirection: 'row' as const, alignItems: 'center' as const, gap: 12, marginBottom: 14 },
  ...
  adjustmentCard: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: colors.surface, borderRadius: 12, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: colors.adjustment + '15' },
  ...
  syncBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.surfaceLight, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
  syncText: { fontSize: 11, fontWeight: '600' as const },
  globalSearchBtn: { backgroundColor: colors.primarySurface, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 },
  ...
  shiftModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'center', alignItems: 'center', padding: 24 },
  shiftModalContent: { backgroundColor: colors.surface, borderRadius: 20, padding: 24, width: '100%', maxWidth: 340 },
  shiftModalTitle: { fontSize: 18, fontWeight: '700' as const, color: colors.text, marginBottom: 6, letterSpacing: -0.2 },
  shiftModalSub: { fontSize: 13, color: colors.textSecondary, marginBottom: 20 },
  shiftModalLabel: { fontSize: 13, fontWeight: '600' as const, color: colors.textSecondary, marginBottom: 8 },
  shiftModalInput: { backgroundColor: colors.surfaceLight, borderRadius: 12, padding: 14, fontSize: 18, fontWeight: '700' as const, color: colors.text, borderWidth: 1, borderColor: colors.border, textAlign: 'center' as const, marginBottom: 18 },
  shiftModalBtns: { flexDirection: 'row', gap: 10 },
  shiftModalCancelBtn: { flex: 1, padding: 14, borderRadius: 12, alignItems: 'center', backgroundColor: colors.surfaceLight },
