...
import {
  View, Text, TouchableOpacity, StyleSheet, Alert, KeyboardAvoidingView, Platform, Animated, ActivityIndicator
} from 'react-native';
...
export default function LoginScreen() {
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        const loggedUser = result.user ?? users.find(u => u.login.toLowerCase() === loginStr.trim().toLowerCase()) ?? users.find(u => u.role === 'admin');
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  logoCircle: { width: 88, height: 88, borderRadius: 28, backgroundColor: colors.primarySurface, alignItems: 'center', justifyContent: 'center', marginBottom: 20, borderWidth: 2, borderColor: colors.primary + '25' },
  title: { fontSize: 28, fontWeight: '800' as const, color: colors.text, letterSpacing: -0.5 },
  subtitle: { fontSize: 14, color: colors.textSecondary, marginTop: 6 },
  form: { width: '100%', maxWidth: 340 },
  inputWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surface, borderRadius: 14, paddingHorizontal: 16, marginBottom: 12, borderWidth: 1, borderColor: colors.border },
  input: { flex: 1, paddingVertical: 16, paddingLeft: 12, fontSize: 15, color: colors.text },
  loginBtn: { backgroundColor: colors.primary, borderRadius: 14, padding: 16, alignItems: 'center', marginTop: 10 },
  ...
  hint: { fontSize: 12, color: colors.textTertiary, marginTop: 36 },
});
