...
import {
  ...
  Tariffs, CashShift, Expense, ClientDebt, MonthlySubscription,
...
function recalculateAllClientDebts(
  ...
  const activeDebtSessions = sessions.filter(
    s => ['active', 'active_debt'].includes(s.status) && !s.exitTime && !s.cancelled
  );
  const activeSessionIds = new Set(activeDebtSessions.map(s => s.id));

  for (const session of activeDebtSessions) {
    const sessionDebt = calculateActiveSessionDebt(session, tariffs, subscriptions, debts);
    if (sessionDebt > 0) {
      clientMap.set(session.clientId, (clientMap.get(session.clientId) ?? 0) + sessionDebt);
    }
  }

  const standaloneDebts = debts.filter(
    d => d.status === 'active' && d.remainingAmount > 0 && !activeSessionIds.has(d.parkingEntryId ?? '')
  );
  for (const debt of standaloneDebts) {
    clientMap.set(debt.clientId, (clientMap.get(debt.clientId) ?? 0) + debt.remainingAmount);
  }

  return Array.from(clientMap.entries()).map(([clientId, total]) => ({
    id: clientId,
    clientId,
    totalAmount: roundMoney(total),
    frozenAmount: 0,
    activeAmount: roundMoney(total),
    lastUpdate: nowStr,
  }));
}

function clientDebtsEqual(a: ClientDebt[], b: ClientDebt[]): boolean {
  if (a.length !== b.length) return false;
  const mapA = new Map(a.map(d => [d.clientId, d.totalAmount]));
  for (const d of b) {
    const aVal = mapA.get(d.clientId);
    if (aVal === undefined || Math.abs(aVal - d.totalAmount) > 1) return false;
  }
  return true;
}
...
function getLocalDateAtHour(dateStr: string, hour: number): Date {
...
type ArrayDataKey = typeof ARRAY_DATA_KEYS[number];
type AppDataRecord = Record<string, unknown>;

function getEntityTimestamp(entity: AppDataRecord): number {
  const value =
    entity.updatedAt ??
    entity.deletedAt ??
    entity.timestamp ??
...
function mergeArrayById<T extends AppDataRecord>(localItems: T[] = [], remoteItems: T[] = []): T[] {
  ...
  const put = (item: T) => {
    ...
    if (!existing || getEntityTimestamp(item) >= getEntityTimestamp(existing)) {
      map.set(id, item);
    }
  };
...
function pickLatestObject<T extends { updatedAt?: string }>(localObj: T, remoteObj: T): T {
  const localTs = localObj?.updatedAt ? new Date(localObj.updatedAt).getTime() : 0;
  const remoteTs = remoteObj?.updatedAt ? new Date(remoteObj.updatedAt).getTime() : 0;
  if (!Number.isFinite(localTs)) return remoteObj;
  if (!Number.isFinite(remoteTs)) return localObj;
  return localTs > remoteTs ? localObj : remoteObj;
}

function mergeAppData(localData: Partial<AppData>, remoteData: Partial<AppData>): AppData {
  const empty = createEmptyData();
  const localFull = { ...empty, ...localData } as AppData;
  const remoteFull = { ...empty, ...remoteData } as AppData;
  const merged = { ...localFull, ...remoteFull } as AppData;

  for (const key of ARRAY_DATA_KEYS) {
    const localItems = (localFull[key] ?? []) as unknown as AppDataRecord[];
    const remoteItems = (remoteFull[key] ?? []) as unknown as AppDataRecord[];
    (merged as unknown as Record<ArrayDataKey, AppDataRecord[]>)[key] = mergeArrayById(localItems, remoteItems);
  }

  merged.tariffs = pickLatestObject(localFull.tariffs, remoteFull.tariffs);
  merged.actionLogs = merged.actionLogs
    .sort((a, b) => getEntityTimestamp(b as unknown as AppDataRecord) - getEntityTimestamp(a as unknown as AppDataRecord))
    .slice(0, 5000);
  merged.loginLogs = merged.loginLogs
    .sort((a, b) => getEntityTimestamp(b as unknown as AppDataRecord) - getEntityTimestamp(a as unknown as AppDataRecord))
    .slice(0, 5000);
  merged.dailyOccupancySnapshots = merged.dailyOccupancySnapshots
...
export const [ParkingProvider, useParking] = createContextHook(() => {
    ...
    void (async () => {
      ...
      if (tableOk) {
        ...
        if (remote && Object.keys(remote).length > 0) {
          ...
          let merged = mergeAppData(createEmptyData(), migrated);
          if (stored) {
            try {
              const localParsed = applyMigrations(JSON.parse(stored) as Partial<AppData>);
              merged = mergeAppData(localParsed, migrated);
            } catch {
              console.log('[Parking] Failed to parse local cache during Supabase load');
            }
          }
          setData(merged);
          void AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
          void saveToSupabase(merged);
          ...
          if (stored) {
            try {
              ...
              const merged = mergeAppData(createEmptyData(), parsed);
        ...
        if (stored) {
          try {
            const parsed = applyMigrations(JSON.parse(stored) as Partial<AppData>);
            setData(prev => mergeAppData(prev, parsed));
          } catch {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToChanges((newData) => {
      ...
      setData(prev => {
        const merged = mergeAppData(prev, migrated);
        if (pendingSave.current) {
          pendingSave.current = mergeAppData(pendingSave.current, merged);
          console.log('[Parking] Merged realtime update into pending local save');
        }
        void AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
        return merged;
      });
  ...
  const persist = useCallback((newData: AppData) => {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      const merged = remote && Object.keys(remote).length > 0
        ? mergeAppData(toSave, applyMigrations(remote))
        : toSave;
      if (merged !== toSave) {
        setData(merged);
        void AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
      }
      const ok = await saveToSupabase(merged);
      if (ok) {
        setSyncStatus('connected');
  ...
  const addClientDraft = useCallback((name?: string, phone?: string, plateNumber?: string, carModel?: string, notes?: string): Client & { carId?: string } => {
    const clientId = generateId();
    const carId = plateNumber?.trim() ? generateId() : undefined;
    const now = new Date().toISOString();

    const client: Client = {
      id: clientId,
      name: name?.trim() || 'Черновик клиента',
      phone: phone?.trim() || '',
      notes: notes ?? '',
      createdAt: now,
      updatedAt: now,
    ...
    update(prev => ({
      ...prev,
      clients: [...prev.clients, client],
      cars: car ? [...prev.cars, car] : prev.cars,
    }));

    logAction('client_draft_add', 'Черновик клиента', `${client.name}, ${client.phone}, ${plateNumber ?? ''}`.trim(), clientId, 'client');
    return { ...client, carId };
  }, [update, logAction]);
  ...
  const addCarToClient = useCallback((clientId: string, plateNumber: string, carModel: string): Car => {
    ...
    update(prev => ({
      ...
      clients: prev.clients.map(c =>
        c.id === clientId && c.isDraft && c.name.trim() && c.phone.trim()
          ? { ...c, isDraft: false, updatedAt: new Date().toISOString() }
          : c
      ),
      cars: [...prev.cars, car],
    }));
  ...
  const updateClient = useCallback((clientId: string, updates: Partial<Pick<Client, 'name' | 'phone' | 'phone2' | 'notes'>>) => {
    ...
    update(prev => ({
      ...
      clients: prev.clients.map(c => {
        ...
        const candidate = { ...c, ...updates };
        const hasCar = prev.cars.some(car => car.clientId === clientId && !car.deleted && car.plateNumber.trim());
        const isCompleteDraft = Boolean(candidate.isDraft && candidate.name.trim() && candidate.phone.trim() && hasCar);
        return { ...candidate, isDraft: isCompleteDraft ? false : candidate.isDraft, updatedAt: now };
      }),
  ...
  const checkIn = useCallback((params: {
    ...
    const finalSessions = [...data.sessions, session];
    const finalDebts = [...newDebts, ...data.debts];
    updatedClientDebts = recalculateAllClientDebts(finalSessions, finalDebts, tariffs, updatedSubscriptions, now);
  ...
  const checkOut = useCallback((sessionId: string, payment?: {
    ...
    if (session.serviceType === 'onetime' && session.status !== 'active_debt') {
    ...
    } else if (session.serviceType === 'monthly' && session.status !== 'active_debt') {
      ...
      const actualDebt = calculateActiveSessionDebt(session, tariffs, data.subscriptions, data.debts);
      console.log(`[CheckOut] active_debt session ${sessionId}: dynamicDebt=${actualDebt}, days=${days}`);
      ...
      if (payment && payment.amount > 0) {
        ...
        newPayments.push({
          ...
          description: `Оплата долга (${days} сут.)`,
          operatorId: currentUser.id, operatorName: currentUser.name,
          date: now, shiftId: currentShift?.id,
        });
        newTransactions.push({
          id: generateId(), type: 'payment', amount: roundMoney(paid),
          description: `Оплата долга (${days} сут.)`, method: payment.method,
          clientId: session.clientId, carId: session.carId,
          operatorId: currentUser.id, operatorName: currentUser.name, date: now,
        });
        if (payment.method === 'cash' && currentShift) {
        ...
        if (payment.method === 'card') {
          newAdminOps.push({
            id: generateId(), type: 'card_income', amount: roundMoney(paid),
            method: 'card', description: 'Безнал при выезде (долг)',
            date: now, operatorId: currentUser.id, operatorName: currentUser.name,
          });
        }
        ...
        if (afterPay > 0) {
          newDebts.push({
            ...
            description: `Остаток долга (${days} сут.)`, createdAt: now,
          });
          newStatus = 'released_debt';
        }
      } else {
        if (actualDebt > 0) {
          newDebts.push({
            ...
            description: `Долг при выезде (${days} сут.)`, createdAt: now,
          });
        }
        newStatus = 'released_debt';
      }

      const oldSessionDebts = data.debts.filter(
        d => d.parkingEntryId === sessionId && d.status === 'active'
      );
      const oldDebtTotal = oldSessionDebts.reduce((s, d) => s + d.remainingAmount, 0);
      const cd = updatedClientDebts.find(c => c.clientId === session.clientId);
      if (cd) {
        updatedClientDebts = updatedClientDebts.map(c =>
          c.clientId === session.clientId
            ? {
                ...c,
                totalAmount: Math.max(0, c.totalAmount - oldDebtTotal),
                activeAmount: Math.max(0, c.activeAmount - oldDebtTotal),
                lastUpdate: now,
              }
            : c
        );
    ...
    const closedSessionDebts = session.status === 'active_debt'
      ? data.debts.filter(d => d.parkingEntryId === sessionId && d.status === 'active').map(d => d.id)
      : [];
    const finalSessions = data.sessions.map(s =>
      s.id === sessionId ? { ...s, status: newStatus, exitTime: now } : s
    );
    const finalDebts = [
      ...newDebts,
      ...data.debts.map(d =>
        closedSessionDebts.includes(d.id)
          ? { ...d, remainingAmount: 0, status: 'paid' as const, updatedAt: now }
          : d
      ),
    ];
    updatedClientDebts = recalculateAllClientDebts(finalSessions, finalDebts, tariffs, data.subscriptions, now);
    ...
    logAction('checkout', 'Выезд', `${car?.plateNumber ?? ''}, статус: ${newStatus}, долг: ${session.status === 'active_debt' ? calculateActiveSessionDebt(session, tariffs, data.subscriptions, data.debts) : 0}`, sessionId, 'session');
  ...
  const cancelCheckIn = useCallback((sessionId: string) => {
    ...
    const finalSessions = data.sessions.map(s =>
      s.id === sessionId ? { ...s, status: 'completed' as const, exitTime: now, cancelled: true } : s
    );
    const finalDailyDebtAccruals = data.dailyDebtAccruals.filter(a => a.parkingEntryId !== sessionId);
    updatedClientDebts = recalculateAllClientDebts(finalSessions, cancelledDebts, data.tariffs, updatedSubs, now);
    ...
    update(prev => ({
      ...
      clientDebts: updatedClientDebts,
      dailyDebtAccruals: finalDailyDebtAccruals,
      transactions: [...newTransactions, ...prev.transactions],
      shifts: updatedShifts,
  ...
  const closeShift = useCallback((actualCash: number, note?: string) => {
    ...
    const summary = calculateShiftClosingSummary(currentShift, data.transactions, data.expenses, data.withdrawals);
    const calculatedBalance = summary.calculatedBalance;
    const discrepancy = roundMoney(actualCash - calculatedBalance);
    if (Math.abs(discrepancy) > 0.01 && !note?.trim()) return;
  ...
  const addExpense = useCallback((amount: number, category: string, description: string): { success: boolean; error?: string } => {
    ...
    if (!description.trim()) return { success: false, error: 'Укажите комментарий к расходу' };
    const availableCash = calculateShiftClosingSummary(currentShift, data.transactions, data.expenses, data.withdrawals).calculatedBalance;
    if (roundMoney(amount) > availableCash) {
      return { success: false, error: `Недостаточно средств в кассе. Доступно: ${availableCash.toFixed(0)} ₽` };
    }
    const now = new Date().toISOString();
    const expense: Expense = {
      id: generateId(), amount: roundMoney(amount), category, description: description.trim(),
      operatorId: currentUser.id, operatorName: currentUser.name,
    ...
    update(prev => ({
      ...
      transactions: [{
        ...
        description: `Расход: ${category} - ${description.trim()}`, method: 'cash' as const,
        operatorId: currentUser.id, operatorName: currentUser.name, date: now,
        shiftId: currentShift.id,
      }, ...prev.transactions],
    }));
    ...
    return { success: true };
  }, [currentUser, currentShift, data.transactions, data.expenses, data.withdrawals, isAdmin, update, logAction]);
  ...
  const payDebt = useCallback((debtId: string, amount: number, method: PaymentMethod) => {
    ...
    const updatedClientDebts = recalculateAllClientDebts(
      data.sessions,
      updatedDebts,
      data.tariffs,
      data.subscriptions,
  ...
  }, [currentUser, data.debts, data.sessions, data.tariffs, data.subscriptions, data.shifts, currentShift, update, logAction]);
  ...
  const withdrawCash = useCallback((amount: number, note?: string): { success: boolean; error?: string } => {
    if (!currentUser || !currentShift) return { success: false, error: 'Нет открытой смены' };
    if (!note?.trim()) return { success: false, error: 'Укажите комментарий к снятию' };
    const withdrawalNote = note.trim();
    const availableCash = calculateShiftClosingSummary(currentShift, data.transactions, data.expenses, data.withdrawals).calculatedBalance;
    if (roundMoney(amount) > availableCash) {
      return { success: false, error: `Недостаточно средств в кассе. Доступно: ${availableCash.toFixed(0)} ₽` };
    }
    ...
    const withdrawal: CashWithdrawal = {
      ...
      operatorId: currentUser.id, operatorName: currentUser.name,
      date: now, shiftId: currentShift.id, note: withdrawalNote,
    };
    ...
    update(prev => ({
      ...
      transactions: [{
        id: generateId(), type: 'withdrawal' as const, amount: roundMoney(amount),
        description: `Снятие: ${withdrawalNote}`, method: 'cash' as const,
        operatorId: currentUser.id, operatorName: currentUser.name, date: now,
        shiftId: currentShift.id,
      }, ...prev.transactions],
    }));
    ...
    return { success: true };
  }, [currentUser, currentShift, data.transactions, data.expenses, data.withdrawals, update, logAction]);
  ...
  const adminWithdrawFromManager = useCallback((amount: number, note?: string): { success: boolean; error?: string } => {
    ...
    if (!note?.trim()) return { success: false, error: 'Укажите комментарий к снятию' };
    const withdrawalNote = note.trim();
    const managerCash = currentShift
      ? calculateShiftClosingSummary(currentShift, data.transactions, data.expenses, data.withdrawals).calculatedBalance
      : 0;
    ...
    const adminOp: AdminCashOperation = {
      ...
      amount: roundMoney(amount),
      method: 'cash',
      description: `Снятие с кассы менеджера: ${withdrawalNote}`,
      date: now,
      operatorId: currentUser.id,
      operatorName: currentUser.name,
    };
    ...
    logAction('withdrawal', 'Снятие из кассы', `${amount} (${withdrawalNote})`, withdrawal.id, 'withdrawal');
    return { success: true };
  }, [currentUser, currentShift, data.transactions, data.expenses, data.withdrawals, update, logAction]);
  ...
  const cancelCheckOut = useCallback((sessionId: string) => {
    ...
    const finalDebts = data.debts.map(d =>
      debtsToCancel.some(dc => dc.id === d.id)
        ? { ...d, remainingAmount: 0, status: 'paid' as const, updatedAt: now }
        : d
    );
    updatedClientDebts = recalculateAllClientDebts(finalSessions, finalDebts, data.tariffs, data.subscriptions, now);
  ...
  const runDebtAccrual = useCallback(() => {
    update(prev => {
      ...
      const finalDebts = updatedDebts ?? prev.debts;
      const finalSessions = prev.sessions;

      const recalcClientDebts = recalculateAllClientDebts(
        finalSessions, finalDebts, prev.tariffs, prev.subscriptions, nowStr
      );

      const hasAccrualChanges = newAccruals.length > 0;
      const hasDebtChanges = updatedDebts !== null;
      const hasClientDebtChanges = !clientDebtsEqual(prev.clientDebts, recalcClientDebts);

      if (hasAccrualChanges || hasDebtChanges || hasClientDebtChanges) {
        if (hasAccrualChanges) {
          console.log(`[DebtAccrual] Added ${newAccruals.length} accruals`);
        }
        if (hasClientDebtChanges) {
          console.log(`[DebtAccrual] clientDebts recalculated`);
        }
        return {
          ...prev,
          dailyDebtAccruals: [...prev.dailyDebtAccruals, ...newAccruals],
          clientDebts: recalcClientDebts,
  ...
  const runOccupancySnapshot = useCallback(() => {
    update(prev => {
      ...
      if (lastSnapshot) {
        const lastDate = getLocalDateAtHour(lastSnapshot.date, 12);
        lastDate.setDate(lastDate.getDate() + 1);
        while (formatLocalDate(lastDate) < todayDateStr) {
          missedDates.push(formatLocalDate(lastDate));
          lastDate.setDate(lastDate.getDate() + 1);
        }
      }
      ...
      const buildSnapshot = (dateStr: string, snapshotTime: string): DailyOccupancySnapshot => {
        const snapshotDate = getLocalDateAtHour(dateStr, 4);
      ...
      for (const missedDate of missedDates) {
        newSnapshots.push(buildSnapshot(missedDate, getLocalDateAtHour(missedDate, 4).toISOString()));
      }
  ...
  }, [currentUser, data.sessions, data.tariffs, data.subscriptions, data.debts, data.shifts, currentShift, update, logAction]);
  ...
  const addManualDebt = useCallback((clientId: string, amount: number, comment: string, carId?: string, lombardAccrual?: boolean) => {
    ...
    const updatedDebts = [...data.debts, debt];
    const updatedClientDebts = recalculateAllClientDebts(
      data.sessions,
      updatedDebts,
      data.tariffs,
    ...
    update(prev => ({
      ...
      transactions: [{
        ...
        description, clientId, carId,
        operatorId: currentUser.id, operatorName: currentUser.name, date: now,
      }, ...prev.transactions],
    }));
    logAction('debt_add', 'Долг добавлен', `${amount}: ${description}${lombardAccrual ? ' (ломбард)' : ''}`, debt.id, 'debt');
  }, [currentUser, data.debts, data.sessions, data.tariffs, data.subscriptions, update, logAction]);
  ...
  const deleteDebt = useCallback((debtId: string) => {
    ...
    logAction('debt_delete', 'Долг удалён', debt?.description ?? '', debtId, 'debt');
  }, [currentUser, data.debts, data.sessions, data.tariffs, data.subscriptions, update, logAction]);
  ...
  const adminForceCloseShift = useCallback((shiftId: string, actualCash: number, note?: string) => {
    ...
    const summary = calculateShiftClosingSummary(targetShift, data.transactions, data.expenses, data.withdrawals);
    const calculatedBalance = summary.calculatedBalance;
    const discrepancy = roundMoney(actualCash - calculatedBalance);
    ...
    update(prev => ({
      ...
      shifts: prev.shifts.map(s =>
        s.id === shiftId ? {
          ...
          cashVarianceType: discrepancy === 0 ? 'none' as const : discrepancy < 0 ? 'short' as const : 'over' as const,
          closingSummary,
          note: `[Закрыта админом] ${closeNote}`,
        } : s
      ),
    }));
    ...
    try {
      ...
      if (currentUser) {
        restored.actionLogs = [{
          ...
          details: 'Данные восстановлены из файла',
          operatorId: currentUser.id,
          operatorName: currentUser.name,
          timestamp: new Date().toISOString(),
        }, ...restored.actionLogs].slice(0, 5000);
      }
      setData(restored);
      persist(restored);
      return { success: true };
  ...
  }, [currentUser, data.users, persist]);
  ...
  const importClientsJson = useCallback((jsonString: string): { success: boolean; error?: string; imported?: number; skipped?: number } => {
    try {
      ...
      const importData = parsed.data as { clients: Client[]; cars: Car[] };
      ...
      let imported = 0;
      let skipped = 0;
      const existingPhones = new Set(data.clients.filter(c => !c.deleted).map(c => normalizePhone(c.phone)));
      const existingPlates = new Set(data.cars.filter(c => !c.deleted).map(c => normalizePlateForSearch(c.plateNumber)));
      ...
      if (importData.cars && Array.isArray(importData.cars)) {
        for (const car of importData.cars) {
          ...
          const plateKey = normalizePlateForSearch(car.plateNumber);
          if (existingPlates.has(plateKey)) continue;
  ...
  const resetAllData = useCallback(() => {
    ...
    if (currentUser) {
      fresh.actionLogs = [{
        id: generateId(),
        action: 'safe_reset',
        label: 'Безопасный reset',
        details: 'Операционные данные очищены, справочники сохранены',
        operatorId: currentUser.id,
        operatorName: currentUser.name,
        timestamp: new Date().toISOString(),
      }];
  ...
  }, [currentUser, data.users, data.tariffs, data.clients, data.cars, data.expenseCategories, data.cleanupChecklistTemplate, persist]);
  ...
  return useMemo(() => ({
    ...
    addClient, addClientDraft, addCarToClient, updateClient, deleteClient, updateCar, deleteCar,
