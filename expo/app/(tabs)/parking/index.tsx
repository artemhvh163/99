...
import { buildEntitySearchQuery, formatMoney, formatDateTime, calculateDays, getServiceTypeLabel, matchCarAndClient } from '@/utils/helpers';
  ...
  const sessions = useMemo(() => {
    return activeSessions
      ...
      .filter(item => {
        ...
        return matchCarAndClient(item.car, item.client, buildEntitySearchQuery(search)).matches;
      })
      .filter((item, _i, arr) => {
        if (!search.trim()) return true;
        const query = buildEntitySearchQuery(search);
        const hasExactInArr = arr.some(it => matchCarAndClient(it.car, it.client, query).exact);
        if (!hasExactInArr) return true;
        return matchCarAndClient(item.car, item.client, query).exact;
      })
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  plate: { fontSize: 17, fontWeight: '700' as const, color: colors.text, letterSpacing: 0.8 },
  ...
  clientNameLink: { fontSize: 14, fontWeight: '500' as const, color: colors.primary, marginBottom: 4, textDecorationLine: 'underline' as const },
  infoRow: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  infoText: { fontSize: 12, color: colors.textTertiary },
  costText: { fontSize: 16, fontWeight: '700' as const, color: colors.primary, marginTop: 8, letterSpacing: -0.2 },
  ...
  cancelBtn: {
    backgroundColor: colors.dangerSurface, borderRadius: 12,
