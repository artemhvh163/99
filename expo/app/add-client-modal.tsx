...
export default function AddClientModal() {
  ...
  const { addClient, addClientDraft } = useParking();
  ...
  const handleSaveDraft = useCallback(() => {
    const cleanPhone = phone.trim() === '+7' ? '' : phone.trim();
    if (!name.trim() && !cleanPhone && !plate.trim() && !model.trim() && !notes.trim()) {
      Alert.alert('Ошибка', 'Заполните хотя бы одно поле для черновика');
      return;
    }
    ...
    addClientDraft(
      ...
      plate.trim() ? formatPlateNumber(plate) : undefined,
      model.trim() || undefined,
      notes.trim() || undefined,
    );
    Alert.alert('Готово', 'Черновик клиента сохранён');
    router.back();
  }, [name, phone, plate, model, notes, addClientDraft, router]);
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  draftBtn: {
    ...
    alignItems: 'center', marginTop: 10, borderWidth: 1, borderColor: colors.border,
  },
  draftBtnText: { fontSize: 16, fontWeight: '700' as const, color: colors.text },
});
